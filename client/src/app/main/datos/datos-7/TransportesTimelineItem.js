...
function TransportesTimelineItem({ item, last }) {

    //funciones

    const retornaTitulo = () => {
        let vehiculo;
        switch (item.especialTransportes.vehiculo) {
            case "camion":
                vehiculo = "Camión"
                break;
            case "trailer":
        ...
        const titulo = "Destino: " + item.especialTransportes.destino + " - Vehículo: " + vehiculo;
        return titulo
    };
                        ...
                        return (
                            <div key={'historico-' + index} className="flex flex-col mt-8 sm:mt-4 text-md leading-5">
                                <Typography className="text-14">
                                    {`Precio: ${cambio.precioUnitario.toLocaleString()} €`}
                                </Typography>
                                <div>
...
export default TransportesTimelineItem;